...
>     proportion of false positives, considering 3 threshold (0.01,
...
>     the mean observed p-value distribution across comparisons, with
...
> Kolmogorov-Smirnov test:
>
> ```{r KSplot, fig.cap="KS plot. Kolmogorov-Smirnov (KS) statistic boxplots for each DA methods where the raw p-value distribution is compared with a uniform distribution.", eval=TRUE, fig.height=4, fig.width=7}
...
>
> The `createConcordance()` method is very flexible. In the example
> below the concordances are built using the log fold changes or other
...
> test. The `plotContingency()` function summarize all these
> information:
>
...
> the user, however we are considering a toy example with only 58
> features.
